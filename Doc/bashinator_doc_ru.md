...
- Проверка форматов (IP, UUID, CIDR) перед сетевыми операциями.
...
Юзкейс: сеть для firewall/ACL.
Вход: строка. Выход: `0` — формат CIDR, `1` — иначе.

```bash
if ! IsCidr "$NETWORK"; then
  printf '%s\n' "CIDR должен быть вида 10.0.0.0/24" >&2
  exit 1
fi
```
...
- `IsIPv6` и `IsCidr` выполняют базовую проверку формата без углубленной
